...
import HTTPManager, { HTTPInterface } from "../../src/assets/js/http_manager";
...
describe("Playlist Editor tests", () => {
  ...
  it("createPlaylist verifies the promise", async () => {
    ...
    try {
      ...
      expect(() => {}).toBeCalledWith(e);
    }
  });

  it("createPlaylist should correctly call getImageInput, getIdFromName", async () => {
    const playListEditorGetImageInputSpy = jest.spyOn(playListEditor, "getImageInput").mockImplementation(() => {});
    const playListEditorGetIdFromNameSpy = jest
      .spyOn(playListEditor, "getIdFromName")
      .mockImplementation(() => null);
    const form = document.getElementById("playlist-form");
    jest.spyOn(form, "elements", "get").mockReturnValue({ name: "", description: "" });
    await playListEditor.createPlaylist(form, () => 0);
    expect(playListEditorGetImageInputSpy).toBeCalled();
    expect(playListEditorGetIdFromNameSpy).toBeCalled();
  });

  it("getIdFromName should return the id element", () => {
    const songElement = [{ id: 0, name: "Whip", artist: "prazkhanal", src: "./assets/media/01_song.mp3", genre: "Electronic", liked: false }];
    playListEditor.songs.push(songElement);
    const idSongs = playListEditor.getIdFromName(songElement.name);
    expect(idSongs).toEqual(songElement.id);
  });
